...
import (
	...
	"github.com/gogohigher/xzrpc/example/protobufcodec/message"
	"log"
	"sync"
	"time"
)
...
func arith(ctx context.Context, xzc *xzrpc.XZClient, action, serviceMethod string, args *message.ArithRequest, i int) {
	...
	if err != nil {
		...
		log.Printf("[No.%d] %s %s success: %d * %d = %d", i, action, serviceMethod, args.A, args.B, reply.C)
	}
